![haicoin logo](http://explorer.haicoin.io/assets/img/haicoin312.png)
...
* [Haicoin Blockchain Explorer](http://explorer.haicoin.io)
...
- [Installation](#installation)
    ...
    - [Go get haicoin](#go-get-haicoin)
    - [Run Haicoin from the command line](#run-haicoin-from-the-command-line)
    - [Show Haicoin node options](#show-haicoin-node-options)
    - [Run Haicoin with options](#run-haicoin-with-options)
...
- [Integrating Haicoin with your application](#integrating-haicoin-with-your-application)
...
This will download `github.com/samoslab/haicoin` to `$GOPATH/src/github.com/samoslab/haicoin`.
...
This image has a `haicoin` user for the haicoin daemon to run, with UID and GID 10000.
...
Access the dashboard: [http://localhost:6969](http://localhost:6969).

Access the API: [http://localhost:6969/version](http://localhost:6969/version).
...
[Haicoin Integration Documentation](INTEGRATION.md)
...
Haicoin URIs obey the same rules as specified in Bitcoin's [BIP21](https://github.com/bitcoin/bips/blob/master/bip-0021.mediawiki).
...
need to start a haicoin node. Since the `cli` integration test requires the rpc interface enabled,
...
#### Server Daemon Mode
The default settings for a haicoin node are chosen for `Server Daemon`, which is typically run from source.
